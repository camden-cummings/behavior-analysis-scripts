...
alphatonum_colflip = {'A': 0, 'B': 1, 'C': 2, 'D': 3, 'E': 4, 'F': 5, 'G': 6, 'H': 7}
...
for file in glob.glob('*matrix*'):
	...
	for a in wells.keys():
		for v3 in range(0, len(wells[a])):
			#NOT FIXED FOR CAMERAS NOT FLIPPED. print(v3,a)#num = ((v3)*8)+1 + alphatonum_col[a]
			# IF YOUR CAMERA IS FLIPPED AROUND, UNCOMMENT BELOW LINE AND COMMENT ABOVE LINE
			num = (alphatonum_colflip[a] * 12) + (12 - v3)
