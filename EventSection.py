...
class EventSection:
    ...
    def __init__(self, namestring, startdate, python):
    ...
    def check_starttime(self, startDateTime):
        if self.starttime < startDateTime:
            print(
                "Error, the start time listed in the sections file is before the real beginning of the file, replacing the start time + 1 min, but check your sections file because it coud indicate a big problem")
    ...
    def process_teensy_str(self, teensystr, msecperframe):
        ...
        for a in range(0, len(annotations)):
            ...
            if annotations[a][0] == 'a':
                ...
                if 100 < int(str(annotations[a][1:]).strip(
                        ";")) < 200:  # NEED TO CHANGE THIS FOR ALL THE Vs SPECIFICALLY BETWEEN 100 and 200, OTHERWISE THEY WILL GO WITH THE ELSE STATEMENT (LIGHT AND OTHER DARKS)
    ...
    # 4:26:00 PM      1       v120 D40 a0.01 f500 d5 p D500 a1 f500 d5 p D455 v200;
    def process_teensy_str_python(self, teensystr, msecperframe):
        annotations = []
        annotationsfinal = []
        teensy = teensystr.split(" ")
        # get all indices that start with D
        ...
        for a in range(0, len(annotations)):
            ...
            # need to divide totalD so that it's in frames, not in milliseconds
            if annotations[a][0] == 'a':
                startframe = int(totalD / msecperframe) + 0
                endframe = startframe + 8
            elif annotations[a][0] == 'v':
                if 100 < int(str(annotations[a][1:]).strip(
                        ";")) < 200:  # NEED TO CHANGE THIS FOR ALL THE Vs SPECIFICALLY BETWEEN 100 and 200, OTHERWISE THEY WILL GO WITH THE ELSE STATEMENT (LIGHT AND OTHER DARKS)
                    startframe = int(totalD / msecperframe) + 22
                    endframe = 262
                else:
                    startframe = int(totalD / msecperframe) + 14
                    endframe = 262
            else:
                print("Error, the events are not acoustic or visual (a or v), or there is a bug")
            annotationsfinal.append(annotations[a].split("%")[0] + "%" + str(startframe) + "%" + str(endframe))
    ...
    def add_event(self, eventtype, eventvoltage, eventtime, msecperframe):
        ...
        if self.type != "time":
            ...
            if self.python:
                annotatedeventlist = self.process_teensy_str_python(stringinput, msecperframe)
            else:
                annotatedeventlist = self.process_teensy_str(stringinput, msecperframe)
            for annotatedevent in annotatedeventlist:
                if eventtype + "_" + str(''.join(str(''.join(eventvoltage.split("."))).split(" "))).strip().strip(
                        ";") + "_" + annotatedevent in self.events.keys():
                    self.events[
                        eventtype + "_" + str(''.join(str(''.join(eventvoltage.split("."))).split(" "))).strip().strip(
                            ";") + "_" + annotatedevent].append(eventtime)
                else:
                    self.events[
                        eventtype + "_" + str(''.join(str(''.join(eventvoltage.split("."))).split(" "))).strip().strip(
                            ";") + "_" + annotatedevent] = [eventtime]
